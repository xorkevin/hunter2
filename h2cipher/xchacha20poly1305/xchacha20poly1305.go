...
func NewConfig() (*Config, error) {
	...
	if _, err := rand.Read(key); err != nil {
		return nil, kerrors.WithMsg(err, "Failed to generate xchacha20-poly1305 key")
	}
	return &Config{
		Key: key,
	}, nil
...
func ParseConfig(params string) (*Config, error) {
	if !strings.HasPrefix(params, "$") {
		return nil, kerrors.WithKind(nil, h2cipher.ErrKeyInvalid, "Invalid xchacha20-poly1305 key")
	}
	...
	if err != nil {
		return nil, kerrors.WithKind(err, h2cipher.ErrKeyInvalid, "Invalid xchacha20-poly1305 key")
	}
	return &Config{
		Key: key,
	}, nil
...
func New(config Config) (*Cipher, error) {
	...
	if err != nil {
		return nil, kerrors.WithMsg(err, "Failed to create xchacha20-poly1305 cipher")
	}
	return &Cipher{
...
func (c *Cipher) Decrypt(ciphertext string) ([]byte, error) {
	if !strings.HasPrefix(ciphertext, "$") {
		return nil, kerrors.WithKind(nil, h2cipher.ErrCiphertextInvalid, "Invalid xchacha20-poly1305 ciphertext")
	}
	...
	if err != nil {
		return nil, kerrors.WithKind(err, h2cipher.ErrCiphertextInvalid, "Invalid xchacha20-poly1305 nonce")
	}
	ciphertextbytes, err := base64.RawURLEncoding.DecodeString(b[3])
	if err != nil {
		return nil, kerrors.WithKind(err, h2cipher.ErrCiphertextInvalid, "Invalid xchacha20-poly1305 ciphertext")
	}
	plaintext, err := c.cipher.Open(nil, nonce, ciphertextbytes, nil)
	if err != nil {
		return nil, kerrors.WithKind(err, h2cipher.ErrCiphertextInvalid, "Failed to decrypt xchacha20-poly1305")
	}
